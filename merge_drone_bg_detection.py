...
class MergingDroneBG(object):
    ...
    def preprocessDroneImg(self, drone_img):
        ...
        rotation_matrix = cv2.getRotationMatrix2D((col/2, row/2), randint(0, 90) - 45, 1)
        new_img2 = cv2.warpAffine(drone_img, rotation_matrix, (col, row), flags=cv2.INTER_LANCZOS4)

        # translation & scaling augmentation
        new_width = int(randint(30, 70)) #by pixel
        new_height = int(new_width)#int(new_width * (int(randint(80, 120)) / 100.0))

        resized_img = cv2.resize(new_img2, (new_width, new_height)) 
        
        final_img = self.applyBlur(resized_img)
    ...
    def applyBlur(self, merged_img):
        ...
        chosen_to_blur = 15#randint(1,100)

        if chosen_to_blur <= 15:
            applied_blur = 0#randint(0,1)
            if applied_blur == 0:
                chosen_sigma = 0.5#randint(1,2)
    ...
    def merge(self):
        ...
        for bg_file in bg_files:
            ...
            bg_img = read_bg_img #self.preprocessBgImg(read_bg_img)
            ...
            while continue_crop:
                ...
                for i in range(0,self.num_drones_in_crop):
                    ...
                    #final_img = self.applyBlur(bg_img_roi)
                    final_img = bg_img_roi
